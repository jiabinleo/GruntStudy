...
window.MobileSelect = (function () {
    ...
    MobileSelect.prototype = {
        ...
        init: function (config) {
            ...
            _this.wheel = getClass(_this.mobileSelect, 'wheel'); //wheel 鏁扮粍
            _this.slider = getClass(_this.mobileSelect, 'selectContainer'); // slider 鏁扮粍
        ...
        renderWheels: function (wheelsData) {
            ...
            var tempHTML = '';
            for (var i = 0; i < wheelsData.length; i++) { //鍒�
                tempHTML += '<div class="wheel"><ul class="selectContainer">';
                if (_this.jsonType) {
                    for (var j = 0; j < wheelsData[i].data.length; j++) { //琛�
        ...
        checkArrDeep: function (parent) { //妫€娴嬪瓙鑺傜偣娣卞害  淇敼 displayJson
            var _this = this;
            if ('childs' in parent && parent.childs.length > 0) {
                _this.displayJson.push(_this.generateArrData(parent.childs)); //鐢熸垚瀛愯妭鐐规暟缁�
                _this.checkArrDeep(parent.childs[0]); //妫€娴嬩笅涓€涓瓙鑺傜偣
            }
        },
        ...
        checkRange: function (index, posIndexArr) {
            ...
            for (var i = 0; i < deleteNum; i++) {
                _this.displayJson.pop(); //淇敼 displayJson
            }
            var resultNode;
        ...
        updateWheels: function () {
            ...
            for (var i = 0; i < _this.displayJson.length; i++) { //鍒�
                (function (i) {
                    var tempHTML = '';
                    if (_this.wheel[i]) {
                        //console.log('鎻掑叆Li');
                        for (var j = 0; j < _this.displayJson[i].length; j++) { //琛�
